...
public class NationCategoryService extends CrudService<NationCategory, Integer> {
    ...
    private final static String index = "nation-category";
    @Value("${urlES}")
    protected String url;

    ObjectMapper objectMapper = new ObjectMapper();

    public NationCategory findOne(Integer id) throws IOException {
        Map<String, Object> query = new HashMap<>();
        query.put("id", id);
        List<NationCategory> esResponse = findAll(query);
        if (esResponse.isEmpty()) return null;
        else return objectMapper.convertValue(esResponse.get(0), NationCategory.class);
    }
    ...
    public Integer count(Map<String, Object> query) throws IOException {
        ...
        jsonNode = objectMapper.readTree(qldsRestTemplate.postForObject(url + "/nation-category/_count", searchSourceBuilder.toString(), String.class));
    ...
    private NationCategory convert(Object item) {
        ...
        return objectMapper.convertValue(tmp, NationCategory.class);
    }
}

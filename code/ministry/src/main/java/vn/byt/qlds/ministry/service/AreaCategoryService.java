...
public class AreaCategoryService extends CrudService<AreaCategory, Integer> {
    ...
    private final static String index = "area-category";
    ObjectMapper objectMapper = new ObjectMapper();

    public AreaCategory findOne(Integer id) throws IOException {
        Map<String, Object> query = new HashMap<>();
        query.put("id", id);
        List<AreaCategory> esResponse = findAll(query);
        if (esResponse.isEmpty()) return null;
        else return objectMapper.convertValue(esResponse.get(0), AreaCategory.class);
    }

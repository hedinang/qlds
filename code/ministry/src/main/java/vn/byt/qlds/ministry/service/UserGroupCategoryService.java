...
@Component
public class UserGroupCategoryService extends CrudService<UserGroupCategory, Integer> {
    @Autowired
    ...
    private final static String index = "user-group-category";
    @Value("${urlES}")
    ...
    public Integer count(Map<String, Object> query) throws IOException {
        ...
        jsonNode = objectMapper.readTree(qldsRestTemplate.postForObject(url + "/user-group-category/_count", searchSourceBuilder.toString(), String.class));
        return jsonNode.findPath("count").intValue();
    }

    public UserGroupCategory findOne(Integer id) throws IOException {
        Map<String, Object> query = new HashMap<>();
        query.put("id", id);
        List<UserGroupCategory> esResponse = findAll(query);
        if (esResponse.isEmpty()) return null;
        else return objectMapper.convertValue(esResponse.get(0), UserGroupCategory.class);
    }

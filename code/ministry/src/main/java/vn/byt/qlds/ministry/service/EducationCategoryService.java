...
@Component
public class EducationCategoryService extends CrudService<EducationCategory, Integer> {
    @Autowired
    ...
    private final static String index = "education";
    @Value("${urlES}")
    protected String url;

    ObjectMapper objectMapper = new ObjectMapper();

    public EducationCategory findOne(Integer id) throws IOException {
        Map<String, Object> query = new HashMap<>();
        query.put("id", id);
        List<EducationCategory> esResponse = findAll(query);
        if (esResponse.isEmpty()) return null;
        else return objectMapper.convertValue(esResponse.get(0), EducationCategory.class);
    }
    ...
    public Integer count(Map<String, Object> query) throws IOException {
        ...
        jsonNode = objectMapper.readTree(qldsRestTemplate.postForObject(url + "/education/_count", searchSourceBuilder.toString(), String.class));
        return jsonNode.path("count").intValue();
    }
    ...
    private EducationCategory convert(Object item) {
        ...
        return objectMapper.convertValue(tmp, EducationCategory.class);
    }

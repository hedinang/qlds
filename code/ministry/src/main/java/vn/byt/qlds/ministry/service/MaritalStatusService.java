...
public class MaritalStatusService extends CrudService<MaritalStatus, Integer> {
    ...
    private final static String index = "marital-status";
    @Value("${urlES}")
    protected String url;

    ObjectMapper objectMapper = new ObjectMapper();

    public MaritalStatus findOne(Integer id) throws IOException {
        Map<String, Object> query = new HashMap<>();
        query.put("maritalCode", id);
        List<MaritalStatus> esResponse = findAll(query);
        if (esResponse.isEmpty()) return null;
        else return objectMapper.convertValue(esResponse.get(0), MaritalStatus.class);
    }
    ...
    public Integer count(Map<String, Object> query) throws IOException {
        ...
        jsonNode = objectMapper.readTree(qldsRestTemplate.postForObject(url + "/marital-status/_count", searchSourceBuilder.toString(), String.class));
    ...
    private MaritalStatus convert(Object item) {
        ...
        return objectMapper.convertValue(tmp, MaritalStatus.class);
    }
}

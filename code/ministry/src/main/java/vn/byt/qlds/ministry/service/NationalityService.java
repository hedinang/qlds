...
public class NationalityService extends CrudService<Nationality, Integer> {
    ...
    private final static String index = "nationality";
    @Value("${urlES}")
    protected String url;

    ObjectMapper objectMapper = new ObjectMapper();

    public Nationality findOne(Integer id) throws IOException {
        Map<String, Object> query = new HashMap<>();
        query.put("nationalityCode", id);
        List<Nationality> esResponse = findAll(query);
        if (esResponse.isEmpty()) return null;
        else return objectMapper.convertValue(esResponse.get(0), Nationality.class);
    }
    ...
    public Integer count(Map<String, Object> query) throws IOException {
        ...
        jsonNode = objectMapper.readTree(qldsRestTemplate.postForObject(url + "/nationality/_count", searchSourceBuilder.toString(), String.class));
    ...
    private Nationality convert(Object item) {
        ...
        return objectMapper.convertValue(tmp, Nationality.class);
    }
}
